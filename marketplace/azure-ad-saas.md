...
The Microsoft cloud-based identity and access management service, [Azure Active Directory](/azure/active-directory/fundamentals/active-directory-whatis) (Azure AD) helps users sign in and access internal and external resources. In the Microsoft commercial marketplace, Azure AD makes transactable SaaS offers easier and more secure for everyone, including publishers, buyers, and users. With Azure AD, publishers can automate the provisioning of users to their software as a service (SaaS) apps, and buyers themselves can manage these provisioned users. 
...
| 8. The Azure AD Provisioning Service communicates changes between Azure AD and the publisher's SaaS application. | [Implement a SCIM endpoint](/azure/active-directory/app-provisioning/use-scim-to-provision-users-and-groups) to receive updates from Azure AD as users are added and removed. | Recommended |
| 9. After the app is permissioned and provisioned, users from the buyer's company can use Azure AD SSO to log in to the publisher's SaaS application. | [Use Azure AD SSO](/azure/active-directory/manage-apps/what-is-single-sign-on) to enable users to sign in once with one account to the publisher's SaaS application. | Recommended |
