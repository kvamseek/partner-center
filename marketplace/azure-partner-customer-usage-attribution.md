...
>   - Customer usage attribution Azure consumption data is not shared externally with partners. 
>
...
If you use Azure Resource Manager APIs, you will need to add your tracking ID per the [instructions below](#use-resource-manager-apis) to pass it to Azure Resource Manager as your code deploys resources. This ID is visible in Partner Center on your Azure Application plan's Technical Configuration page. 
...
Unlike the tracking IDs that Partner Center creates on your behalf for Azure apps in the commercial marketplace, other uses of Customer Usage Attribution require you to create a GUID to use as your tracking ID. A GUID is a unique reference identifier that has 32 hexadecimal digits. To create GUIDs for tracking, you should use a GUID generator, for example, via PowerShell:
...
> [!TIP]
> For more information on creating and publishing Resource Manager templates, see: [create and deploy your first Resource Manager template](/azure/azure-resource-manager/templates/quickstart-create-templates-use-the-portal).
...
In some cases, you may make calls directly against the Resource Manager REST APIs to deploy Azure services. [Azure supports multiple SDKs](/azure/) to enable these calls. You can use one of the SDKs or call the REST APIs directly to deploy resources.
...
1. Sign in using your MSA (Microsoft AI Cloud Partner Program account) or your AAD (Partner Dashboard account).
...
View step-by-step instructions with screenshots at [Using Technical Presales and Deployment Services](../technical-benefits.md).
...
#### Does Microsoft share Customer Usage Attribution data with partners? 

No. Microsoft does not share Azure consumption data tracked via Customer Usage Attribution externally with Partners. 

#### Do customers receive Customer Usage Attribution reporting?

No. Customers can track their usage of all resources or resource groups within the Azure portal. Customers do not see usage broken out by Customer Usage Attribution tracking ID.
