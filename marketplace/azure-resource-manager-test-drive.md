...
If you are unfamiliar with what an ARM template is, read [What is Azure Resource Manager?](/azure/azure-resource-manager/management/overview) and [Understand the structure and syntax of ARM templates](/azure/azure-resource-manager/templates/syntax) to better understand how to build and test your own templates.
...
- **Test drive Azure Resource Manager template** – Upload the .zip containing your Azure Resource Manager template. Learn more about creating an Azure Resource Manager template in the quickstart article [Create and deploy Azure Resource Manager templates by using the Azure portal](/azure/azure-resource-manager/templates/quickstart-create-templates-use-the-portal).

    > [!note]
    > To publish successfully, it is important to validate the formatting of the ARM template. Two ways to do this are (1) by using an [online API tool](/rest/api/resources/deployments/validate) or (2) with a [test deployment](/azure/azure-resource-manager/templates/deploy-portal).
...
:::image type="content" source="media/test-drive/resource-manager-parameters.png" alt-text="List of parameters in an Azure Resource Manager":::
...
:::image type="content" source="media/test-drive/deployment-subscription-details.png" alt-text="Test drive deployment subscription details":::
...
1. Obtain an **Azure Subscription ID**. This grants access to Azure services and the Azure portal. The subscription is where resource usage is reported and services are billed. If you do not already have a separate Azure subscription for test drives only, make one. You can find Azure Subscription IDs (such as `1a83645ac-1234-5ab6-6789-1h234g764ghty1`) by signing in to Azure portal and selecting **Subscriptions** from the left-nav menu.
   ...
   :::image type="content" source="media/test-drive/azure-subscriptions.png" alt-text="Azure Subscriptions":::
...
2. Obtain an **Azure AD Tenant ID**. If you already have a Tenant ID available you can find it in **Azure Active Directory** > **Properties** > **Directory ID**:
   ...
   :::image type="content" source="media/test-drive/azure-active-directory-properties.png" alt-text="Azure Active Directory properties":::

   If you don't have a tenant ID, create a new one in Azure Active Directory. For help with setting up a tenant, see [Quickstart: Set up a tenant](/azure/active-directory/develop/quickstart-create-new-tenant).
...
3. Provision the Microsoft Test-Drive application to your tenant. We will use this application to perform operations on your test drive resources.
    ...
    1. Add the Service Principal for Microsoft Test-Drive application.
      ...
      :::image type="content" source="media/test-drive/commands-to-verify-service-principal.png" alt-text="Shows the code to verify service principal":::
...
1. Verify that your test drive is deploying correctly.
   ...
   :::image type="content" source="media/test-drive/azure-portal.png" alt-text="Azure portal":::
