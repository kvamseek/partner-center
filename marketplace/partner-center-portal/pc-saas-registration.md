---
title: Register a SaaS application 
...
For more information about Azure AD (Active Directory), see [What is authentication](/azure/active-directory/develop/authentication-vs-authorization).
...
3. In the left-hand navigation pane, select the **Azure Active Directory** service, select **App registrations**, and then select **New application registration**.
    ...
    :::image type="content" source="./media/saas-offer-app-registration-v1.png" alt-text="SaaS AD App Registrations":::
...
4. On the Create page, enter your application\'s registration information:
   ...
   - **Application type**:
     ...
     Select **Web app / API** for [client applications](/azure/active-directory/develop/developer-glossary#client-application)) and [resource/API applications](/azure/active-directory/develop/developer-glossary#resource-server)) that are installed on a secure server. This setting is used for OAuth confidential [web clients](/azure/active-directory/develop/developer-glossary#web-client)) and public [user-agent-based  clients](/azure/active-directory/develop/developer-glossary#user-agent-based-client)). The same application can also expose both a client and resource/API.

     For specific examples of web applications, check out the quickstart guided setups that are available in the [Get Started](/azure/active-directory/develop/quickstart-create-new-tenant) section of the [Azure AD Developers Guide](/azure/active-directory/develop/).
...
For more information about these tokens, see [Azure Active Directory access tokens](/azure/active-directory/develop/access-tokens).  In the flow below V1 endpoint token is used.
