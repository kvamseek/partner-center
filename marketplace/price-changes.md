...
Once publishing is complete, you'll receive an email with the effective date of the new price.
...
Both existing and new customers are billed the new price for any usage consumed after the new price is in effect, regardless of contract duration.
...
6. To change prices specific for a market, you have two options:
   - **Do the conversion yourself:**
     ...
     3. Update the price per hour for each vCPU size.
     4. Repeat if you want to update prices for several markets.
   - **Export to a spreadsheet:**
     1. Export the prices using **Export pricing data**.
     2. Update the market and vCPU size prices in the downloaded spreadsheet and save it.
     3. Import the spreadsheet using **Import pricing data**.
7. To change prices across all markets:
   1. **Per vCPU**: Edit the price per vCPU in the **USD/hour** box.
   2. **Per vCPU size**: Edit each vCPU size in the **Price per hour in USD** box.
   > [!NOTE]
   > If the plan is available in multiple markets, the new price for each market is calculated according to current exchange rates.
8. Select **Save draft**.
9. Confirm you understand the effects of changing the price by entering the **ID of the plan**.
